...
pub(crate) fn dev_cache_dir() -> Result<PathBuf> {
    if let Ok(cache_dir) = env::var("SCIE_PIKESQUARES_DEV_CACHE") {
        let cache_path = PathBuf::from(cache_dir);
        ensure_directory(&cache_path, false)?;
        return cache_path.canonicalize().with_context(|| {
            format!(
                "Failed to resolve the absolute path of SCIE_PIKESQUARES_DEV_CACHE={cache_dir}",
