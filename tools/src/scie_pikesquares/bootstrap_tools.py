...
from scie_pikesquares import INSTALL_URL, VERSION
from scie_pikesquares.log import fatal
...
def versioned(func) -> Callable:
    def wrapper(*args, **kwargs) -> Any:
        version = os.environ.get("PIKESQUARES_BOOTSTRAP_TOOLS")
        if not version:
            fatal(
                f"The {func.__name__} command requires PIKESQUARES_BOOTSTRAP_TOOLS be set in the "
                f"environment."
            )
        ...
        try:
            ...
            fatal(
                ...
                f"PIKESQUARES_BOOTSTRAP_TOOLS={version}"
            )
...
def main() -> NoReturn:
    ...
    parser.add_argument(
        "--pikesquares-version",
        # The version of PikeSquares being used.
        help=argparse.SUPPRESS,
    )
    ...
    cache_key_parser = sub_commands.add_parser(
        ...
        help=(
            ...
            "pikesquares bootstrap directories. (Added in bootstrap version 1.)"
        ),
    )
